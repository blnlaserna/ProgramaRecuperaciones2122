class Tablero {
    ...
    crearTableroVacio() {
        // Listado de celdas vacías del tablero
        this.listaCeldas = [];
    ...
    dibujarDOM(elemento) {
        ...
        for (let i=0; i<this.numFilas; i++) {
            ...
            for (let j=0; j<this.numColumnas; j++) {
                ...
                celda.appendChild(document.createTextNode(`${this.listaCeldas[i][j].getValor()}`));
    ...
    dibujarDOMTableroOculto(elemento) {
        let tabla = document.createElement(`table`);
        let fila;
        let celda;
        let boton;
...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    let tabla = tableroBuscaminas.dibujarDOM(body);
    
    body.removeChild(tabla);
    tableroBuscaminas.recalcular(5,5,10);
    tableroBuscaminas.dibujarDOM(body);
});